...
public:
  ...
  std::map<int, std::list<std::pair<int, std::string>>> GetGraph();
  void AddVertex(const int& vertex);
  void AddEdges(std::vector<std::vector<std::string>> vect);
  void DFS(const int &vertex, std::map<int, bool> &visited) const;
  void DFS(const int &vertex, std::map<int, bool> &visited, const std::string& relationship) const;
  void DFS(const int &vertex, std::map<int, bool> &visited, const std::vector<std::string>& relationships) const;

private:
  std::map<int, std::list<std::pair<int, std::string>>> graph_;
};

void PrintList(std::list<std::pair<int, std::string>>);
